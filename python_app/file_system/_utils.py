...
def getCurrSectionMoveNumber():
    ...
    contentFileMoveNumber = \
        sfs.SectionInfoStructure.readProperty(currSection, 
                                sfs.SectionInfoStructure.PubProp.imageContentFileMoveLinesNumber_ID)
    tocFileMoveNumber = \
        sfs.SectionInfoStructure.readProperty(currSection, 
                                sfs.SectionInfoStructure.PubProp.imageTOCFileMoveLinesNumber_ID)

    if contentFileMoveNumber != _u.Token.NotDef.str_t and tocFileMoveNumber != _u.Token.NotDef.str_t:
        log.autolog("Got move numbers from section. Content: " 
                    + str(contentFileMoveNumber) 
                    + ". TOC: " 
                    + str(tocFileMoveNumber))
    ...
    contentFileMoveNumber = \
        bfs.BookInfoStructure.readProperty(bfs.BookInfoStructure.PubProp.imageContentFileMoveLinesNumber_ID)
    tocFileMoveNumber = \
        bfs.BookInfoStructure.readProperty(bfs.BookInfoStructure.PubProp.imageTOCFileMoveLinesNumber_ID)
    
    log.autolog("Got move numbers from book. Content: " 
                + str(contentFileMoveNumber) 
                + ". TOC: " 
                + str(tocFileMoveNumber))
