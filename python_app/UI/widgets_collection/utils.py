...
class EntryFrameManager(TOCFrame):
...
class EntryImagesFactory:
    ...
    def produceEntryMainImageWidget(self,
        ...
        if not fsf.Data.Sec.isVideo(self.subsection):
            openOMOnThePageOfTheImage(imLabel, self.subsection, self.imIdx)
        else:
            openVideoOnThePlaceOfTheImage(imLabel, self.subsection, self.imIdx)
        return tempLabel

    def __produceEntryExtraImageExtraLabels(self, eImIdx, parentLabel, 
                                            mainFrame, entryWidget, resizeFactor):
        ...
        removeEntry.tocFrame = mainFrame
        ...
        def delEIm(event, *args):
            ...
            pdfReadersManager.show(changePrevPos = False, removePrevLabel = True, 
                                    subsection = self.subsection, imIdx = self.imIdx,
        ...
        moveEntryDown.tocFrame = mainFrame
        ...
        moveEntryUp.tocFrame = mainFrame
        ...
        if mainFrame != None:
            if "showImagesLabels" in dir(mainFrame):
                if mainFrame.showImagesLabels.get(self.subsection + self.imIdx) != None:
                    shownImage = mainFrame.showImagesLabels[self.subsection + self.imIdx]
        ...
        changeImSize = ImageSize_ETR(tempEImLabel,
                                    ...
                                    row = eImIdx + 5, 
                                    column = 4,
                                    imIdx = self.imIdx + "_" + str(eImIdx),
                                    text = resizeFactor)
        changeImSize.imIdx = self.imIdx + "_" + str(eImIdx)
        changeImSize.subsection = self.subsection
        changeImSize.rebind([ww.currUIImpl.Data.BindID.Keys.enter],
                                [lambda e, *args: resizeEntryImgCMD(e, mainFrame)])
        changeImSize.render()
        ...
        bookCodeProj.tocFrame = mainFrame
        ...
        subsectionCodeProj.tocFrame = mainFrame
        ...
        entryCodeProj.tocFrame = mainFrame
        ...
        if self.imIdx in list(fsf.Data.Sec.extraImagesDict(self.subsection).keys()):
            ...
            for eImIdx in range(0, len(extraImages)):
                ...
                if resizeFactor == None:
                    ...
                    if (self.imIdx + "_" + str(eImIdx)) in list(uiResizeEntryIdx.keys()):
                ...
                #NOTE: what is this?????
                mainRow = eImIdx + 5 if row == None else row

                if type(rootLabel) == list:
                    rootLabel = rootLabel[eImIdx]

                tempLabel = EntryImagesFactory.__ExtraImageFrame(extraImFilepath,
                                                                    self.subsection,
                                                                    self.imIdx,
                                                                    eImIdx,
                ...
                eimLabel = self.__getImageWidget(tempLabel, extraImFilepath, eImWidgetName, 
                                            self.imIdx, self.subsection, imPad = 0,
                                            row = 0, column = 1, columnspan = 1,
                                            resizeFactor = resizeFactor,
                                            extraImIdx = eImIdx,
                ...
                openOMOnThePageOfTheImage(eimLabel, self.subsection, self.imIdx, str(eImIdx))
                ...
                if createExtraWidgets:
                    tempEImLabel = self.__produceEntryExtraImageExtraLabels(eImIdx = eImIdx, 
...
class EntryWidgetFactory:
    class __EntryUIs:
        class __EntryUIData:
            def __init__(self, name, column, cmd) -> None:
                self.name = name
                self.column = column
                self.cmd = cmd
        ...
        def __init__(self):
            ...
            self.addExtra = self.__EntryUIData("[Add exta]", 6, EntryWidgetFactory.produceAddExtraImageWidget)
    ...
    def __init__(self, subsection, imIdx, parentWidget, scrollableFrame, topPad, leftPad):
        ...
        self.__entryUIs = self.__EntryUIs()

        self.entryFrameManager = self.__produceEntryWidgetFrames(topPad = topPad, leftPad = leftPad)
    ...
    def produceShiftLabelWidget(self, parentWidget):
        def shiftEntryCmd(event, *args):
            ...
            self.scrollableFrame.renderWithScrollToEntry(widget.subsection,
                                                         widget.imIdx)
    ...
    def produceRemoveEntryWidget(self, parentWidget):
        def removeEntryCmd(event, *args):
            ...
            def __afterDeletion(subsection, imIdx, *args):
                ...
                pdfReaderManager = dt.AppState.UIManagers.getData(self.scrollableFrame.appCurrDataAccessToken,
                                                                    wf.Wr.MenuManagers.PdfReadersManager)
                pdfReaderManager.show(subsection = subsection, 
                                        imIdx = str(imIdx), 
                                        removePrevLabel = True)
    ...
    def produceShowLinksForEntry(self, parentWidget):
        def showLinksForEntryCmd(linksFrame):
        ...
        showLinksForEntry.rebind([ww.currUIImpl.Data.BindID.mouse1],
                                    [lambda e, lf = self.linkFrame, *args: showLinksForEntryCmd(lf)])
    ...
    def produceRetakeImageWidget(self, parentWidget):
        def retakeImageCmd(event, *args):
            ...
            def __cmdAfterImageCreated(self):
                ...
                self.scrollableFrame.renderWithScrollToEntry(widget.subsection, widget.imIdx)
    ...
    def produceOpenExcercisesWidget(self, parentWidget):
        ...
        excerciseExists = entryStructureExists
    ...
    def produceOpenSecondaryImageWidget(self, parentWidget):
        def openSecondaryImage(widget):
            def __cmd(event = None, *args):
                ...
                print(wd.Data.Reactors.entryChangeReactors.keys())
...
Do you want to move group \n\nto subsection\n'{0}' \n\nand entry: \n'{1}'\n\n with group name \n'{2}'?".format(targetSubsection, 
    ...
    def __produceEntryWidgetFrames(self, topPad, leftPad):
        ...
        entryFrameManager.groupFrame.render()
        ...
        entryFrameManager.rowFrame2.render()
    ...
    def produceEntryWidgetsForTOCFrame(self):
        mainImageWidget = self.produceMainImageWidget(parentWidget = self.entryFrameManager.rowFrame1)
        mainImageWidget.render()

        full = self.__entryUIs.full.cmd(self, parentWidget = self.entryFrameManager.rowFrame1)
        self.entryFrameManager.fullMoveWidget = full
        full.render()
        im = self.__entryUIs.im.cmd(self, parentWidget = self.entryFrameManager.rowFrame1)
        im.render()
        copy = self.__entryUIs.copy.cmd(self, parentWidget = self.entryFrameManager.rowFrame1)
        copy.render()
        pasteAfter = self.__entryUIs.pasteAfter.cmd(self, parentWidget = self.entryFrameManager.rowFrame1)
        pasteAfter.render()
        excercises = self.__entryUIs.excercises.cmd(self, parentWidget = self.entryFrameManager.rowFrame1)
        excercises.render()
        copyLink = self.__entryUIs.copyLink.cmd(self, parentWidget = self.entryFrameManager.rowFrame1)
        copyLink.render()
        pasteLink = self.__entryUIs.pasteLink.cmd(self, parentWidget = self.entryFrameManager.rowFrame1)
        pasteLink.render()

        # leadingEntry = self.__entryUIs.leadingEntry.cmd(self, parentWidget = self.frameRow2)
        # delete = self.__entryUIs.delete.cmd(self, parentWidget = self.frameRow2)
        # shift = self.__entryUIs.shift.cmd(self, parentWidget = self.frameRow2)
        # copyText = self.__entryUIs.copyText.cmd(self, parentWidget = self.frameRow2)
        # retake = self.__entryUIs.retake.cmd(self, parentWidget = self.frameRow2)
        # addExtra = self.__entryUIs.addExtra.cmd(self, parentWidget = self.frameRow2)
        # addProof = self.__entryUIs.addProof.cmd(self, parentWidget = self.frameRow2)
        # group = self.__entryUIs.group.cmd(self, parentWidget = self.frameRow2)
        # showSubentries = self.__entryUIs.showSubentries.cmd(self, parentWidget = self.frameRow2)
        # alwaysShow = self.__entryUIs.alwaysShow.cmd(self, parentWidget = self.frameRow2)
        # changeImSize = self.__entryUIs.changeImSize.cmd(self, parentWidget = self.frameRow2)
        # showLinks = self.__entryUIs.showLinks.cmd(self, parentWidget = self.frameRow2)


    def produceEntryWidgetsForEntryFrame(self, topPad, leftPad):
        self.__produceEntryWidgetFrames(topPad = topPad, leftPad = leftPad)

        # mainImageWidget = self.produceMainImageWidget(parentWidget = self.frameRow1)
        # full = self.__entryUIs.full.cmd(parentWidget = self.frameRow1)
        # im = self.__entryUIs.im.cmd(parentWidget = self.frameRow1)
        # copy = self.__entryUIs.copy.cmd(parentWidget = self.frameRow1)
        # pasteAfter = self.__entryUIs.pasteAfter.cmd(parentWidget = self.frameRow1)
        # excercises = self.__entryUIs.excercises.cmd(parentWidget = self.frameRow1)
        # copyLink = self.__entryUIs.copyLink.cmd(parentWidget = self.frameRow1)
        # pasteLink = self.__entryUIs.pasteLink.cmd(parentWidget = self.frameRow1)
        # notes = self.__entryUIs.note.cmd(parentWidget = self.frameRow2)

        # leadingEntry = self.__entryUIs.leadingEntry.cmd(parentWidget = self.frameRow2)
        # delete = self.__entryUIs.delete.cmd(parentWidget = self.frameRow2)
        # shift = self.__entryUIs.shift.cmd(parentWidget = self.frameRow2)
        # copyText = self.__entryUIs.copyText.cmd(parentWidget = self.frameRow2)
        # retake = self.__entryUIs.retake.cmd(parentWidget = self.frameRow2)
        # addExtra = self.__entryUIs.addExtra.cmd(parentWidget = self.frameRow2)
        # addProof = self.__entryUIs.addProof.cmd(parentWidget = self.frameRow2)
        # group = self.__entryUIs.group.cmd(parentWidget = self.frameRow2)
        # showSubentries = self.__entryUIs.showSubentries.cmd(parentWidget = self.frameRow2)
        # alwaysShow = self.__entryUIs.alwaysShow.cmd(parentWidget = self.frameRow2)
        # changeImSize = self.__entryUIs.changeImSize.cmd(parentWidget = self.frameRow2)
        # showLinks = self.__entryUIs.showLinks.cmd(parentWidget = self.frameRow2)
